...
from calculator.evaluation.evaluate import sum_multiple_operators, detect_operators
...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.backspace= QPushButton('C', self)
    ...
    def initui(self):
        ...
        # Display Result
        # self.digit_.clicked.connect(lambda: self.output2.clear() if self.output2.text() else self.output2.insert(self.perform_calculations(self.output.text())))

        self.digit_.clicked.connect(lambda: self.output.insert(self.perform_calculations(self.output.text()))) # if self.output.text() and self.output.clear() )
    ...
    def no_string_input(self):
        pass

    def get_equations(self, input_data):
        operation = ["+", "-", "/", "*"]
        input_list = input_data.split(" ")
        return input_list

    def perform_calculations(self, user_input):
        try:

            self.output.clear()
            self.output2.clear()
            equation_list = self.get_equations(user_input)
            useage, operator = detect_operators(equation_list)
            results = sum_multiple_operators(equation_list)
            return str(f"{results:,}")

        except Exception as e:
            self.output2.insert(str(e))
